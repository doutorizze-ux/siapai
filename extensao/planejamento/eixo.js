window.SIAPEixo = (() => {
  const STORAGE_KEY_VALUE = 'tm_gpt_ultimo_ddlEixo_value';
  const STORAGE_KEY_TEXT = 'tm_gpt_ultimo_ddlEixo_text';
  const STORAGE_KEY_AT = 'tm_gpt_ultimo_ddlEixo_at';
  const SELECTOR = '#ddlEixo, select[id$="ddlEixo"], select[name$="$ddlEixo"]';

  function log(message, ...args) {
    try {
      if (window.SIAPLogger?.log) window.SIAPLogger.log(message);
      else console.log('[SIAP eixo]', message, ...args);
    } catch (_) {}
  }
  ...
  async function restoreLastEixo(options = {}) {
    ...
    if (!hasOptionValue(select, savedValue)) {
      ...
      log(`[Eixo] O último eixo salvo${savedText ? ' (' + savedText + ')' : ''} não existe nesta tela/disciplina. Mantendo o eixo atual.`);
      return false;
    }

    return selectEixo({ value: savedValue, text: getSavedText() }, options);
  }

  async function selectEixo(target = {}, options = {}) {
    ...
    if (!option) {
      log(`[Eixo] Eixo solicitado não existe nesta tela: ${text || value || '-'}`);
      return false;
    }
    ...
    if (options.remember !== false) saveValue(selectedValue, selectedText);
    log(`[Eixo] Eixo selecionado automaticamente: ${selectedText || selectedValue}${beforeText ? ' (antes: ' + beforeText + ')' : ''}`);
  ...
  function bindChangeWatcher() {
    if (window.__SIAP_EIXO_WATCHER_BOUND__) return;
    window.__SIAP_EIXO_WATCHER_BOUND__ = true;
